...
// \n -> nothing, \n\n -> \n, \n\n\n -> \n\n, etc.
fun String.rearrangeNewlines(): String {
    return (0 until length).map {
        if (it == 0) {
            return@map this[0] + ""
        }
        if (this[it] == '\n' && this[it - 1] != '\n') {
            if (it < length - 1 && this[it + 1] == '\n') {
                return@map ""
            }
            return@map " "
        }
...
// assumes iterable sums up to 1, and returns random index based on this distrubution
fun Iterable<Double>.sampleIndexFromThisProbabilityDistribution(): Int {
    var i = 0
    val threshold = defaultRandom.nextDouble() * this.sum()
    var sum = 0.0
    for (p in this) {
        sum += p
        if (sum >= threshold) {
    ...
    throw IllegalArgumentException("probability distribution didn't work")
}
...
val ALPHABET_LOWER = "abcdefghijklmnopqrstuvwxyz".map { it + "" }.toArrayList()
val ALPHABET_UPPER = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".map { it + "" }.toArrayList()
val ALPHABET = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".map { it + "" }.toArrayList()
...
fun Float.format(nDigitsAfterPeriod: Int) = "%.${nDigitsAfterPeriod}f".enFormat(this)
fun Double.format(nDigitsAfterPeriod: Int) = "%.${nDigitsAfterPeriod}f".enFormat(this)
fun String.enFormat(vararg args: Any) = this.format(Locale.ENGLISH, *args)
