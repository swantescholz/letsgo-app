...
object Preferences {
    ...
    val cheatingEnabled: Boolean get() = cellMeterWidth.get() == 42
    val mute = Preference("mute", false, { it.toBoolean() })
    val trainingMode = Preference("trainingMode", false, { it.toBoolean() })
    val vsAiMode = Preference("vsAiMode", true, { it.toBoolean() }) // true -> against  AI, false -> two humans play
    val lastPlayedLevelId = Preference("lastPlayedLevelId", -1, { it.toInt() })
    val cellMeterWidth = Preference("cellMeterWidth", 30, { it.toInt() })
