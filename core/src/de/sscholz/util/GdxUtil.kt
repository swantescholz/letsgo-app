...
import de.sscholz.extensions.beginEndBlock
import ktx.math.div
...
object GdxUtil {
    ...
    fun drawTextureWithGivenHeight(texture: TextureRegion, textUnitHeight: Float, center: Vector2) {
        Global.defaultSpriteBatch.beginEndBlock {
            this.projectionMatrix = camera2.combinedMatrix
            val width = textUnitHeight / texture.regionHeight * texture.regionWidth.toFloat()
            draw(texture, center.x - width / 2f, center.y - textUnitHeight / 2f,
                    width / 2f, textUnitHeight / 2f,
                    width, textUnitHeight, 1f, 1f, 0f)
        }
    }
    ...
    fun getCurrentWorldCoordinatesOfMouse(): Vector2 {
        return camera2.screenToWorldCoordinates(Gdx.input.x.toFloat(),
                Gdx.input.y.toFloat())
    }
