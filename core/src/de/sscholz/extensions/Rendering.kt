...
fun Label.mySetBackgroundColor(color: Color, padding: Float) {
    val labelColor = Pixmap(width.i, height.i, Pixmap.Format.RGBA8888)
    labelColor.setColor(color)
    labelColor.fill()
    val newStyle = LabelStyle(style)
    newStyle.background = Image(Texture(labelColor)).drawable
    newStyle.background.leftWidth = padding
    newStyle.background.rightWidth = padding
    newStyle.background.topHeight = padding
    newStyle.background.bottomHeight = padding
    style = newStyle
    labelColor.dispose()
}
