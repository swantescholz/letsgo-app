...
fun Vector2.min(other: Vector2): Vector2 {
    return vec2(Math.min(x, other.x), Math.min(y, other.y))
}

fun Vector2.max(other: Vector2): Vector2 {
    return vec2(Math.max(x, other.x), Math.max(y, other.y))
}
